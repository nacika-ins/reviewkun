...
var spreadsheet = SpreadsheetApp.openById(
    property.getProperty("spreadsheet_id")
);
...
namespace web {
    export function doPost(e) {
        ...
        return HtmlService.createHtmlOutput(
            JSON.stringify({ repoName, action, content, configs })
        );
    }
    ...
    function postToSlack(repoName, content, config) {
        ...
        var title =
            "<" + content.target.html_url + "|" + content.target.title + ">";
        var botName = "<" + spreadsheet.getUrl() + "|ReviewKun>";
        var message = config.message.replace(
            /{{assignee}}/g,
            (content.target.assignees[0] || {}).login
        ).replace(
            /{{requested_reviewers}}/g,
            content.target.requested_reviewers.map((v) => v.login).join(" ")
        );
        var text =
            label +
            " (by " +
    ...
    function needToPost(action, content, config) {
        ...
        if (
            !content.label.name.match(
                new RegExp(config.labelCondition || "レビュー")
            )
        ) {
            return false;
        }
